...
def run_sml(i):
    for j in K_VALS: # for each k
        for _ in range(500): # every k, 500 times each
            process = Popen([f'./main', f'../testing/40-200kb/combine.csv', f'1000', f'../testing/40-200kb/input/data_{i}_0.csv', f'{j}', f'1000'], stdout=PIPE)
...
def run_med(i):
    for j in K_VALS: # for each k
        for _ in range(500): # every k, 500 times each
            process = Popen([f'./main', f'../testing/10-100mb/combine.csv', f'1000', f'../testing/10-100mb/input/data_{i}_0.csv', f'{j}', f'1000'], stdout=PIPE)
            (output, err) = process.communicate()
            exit_code = process.wait()
            output = output.rstrip().split()
...
def run_lrg(i):
    for j in K_VALS: # for each k
        for _ in range(500): # every k, 500 times each
            process = Popen([f'./main', f'../testing/100mb-1gb/combine.csv', f'1000', f'../testing/100mb-1gb/input/data_{i}_0.csv', f'{j}', f'1000'], stdout=PIPE)
            (output, err) = process.communicate()
            exit_code = process.wait()
            output = output.rstrip().split()
